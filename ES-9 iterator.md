...
function makeIterator(array) {
  var nextIndex = 0;
  return {  //返回一个对象，该对象封装了next方法
    next: function() {
      return nextIndex < array.length ?
        {value: array[nextIndex++], done: false} :  //如果没遍历完，则返回一个对象，里面包含l两个值，当前的数据，和是否完成的状态
...
上面代码定义了一个`makeIterator`函数，它是一个遍历器生成函数**，作用就是返回一个遍历器对象**。对数组`['a', 'b']`执行这个函数，就会返回该数组的**遍历器对象**（即指针对象）`it`。
...
- **Array**  （set方法可以传入数组作为参数，可以看出array实现了iterator接口）
- **Map**
- **Set**
- **String**
- **TypedArray**（类数组）
...
>输出：
>
>![mark](http://qiniu.wind-zhou.com/blog/210401/mIgKiE5g2L.png?imageslim)
>
>![mark](http://qiniu.wind-zhou.com/blog/210401/iKfCEgeL5D.png?imageslim)
>
>
>
>到此为止，然我们对iterator接口做简单总结：
>
>所谓的实现的**Iterator 接口**的目的，就是为所有数据结构，提供了一种统一的访问机制，即使用for...of进行循环遍历 。
>
>那么怎样才叫实现了iterator借口呢，其实只要看该数据结构内部有无Symbol.iterator属性。其实我感觉这里称为Symbol.iterator**属性**不是很正确，这明显是该数据结构内的**方法**呀，只不过方法名是`Symbol.iterator`(是一个Symbol类型的方法名)，**调用该方法会返回一个对象，这个对象称之为遍历器对象**,就像下面这段代码。
>
>```js
>const obj = {
>  [Symbol.iterator] : function () {
...
>```
>
>遍历器对象中最核心的是next方法，他可以改变数据结构内的指向，并实现遍历。
>
>只要一个数据结构实现了iterator接口，就可以使用next方法去操作，当然每种数据结构next方法的具体实现也不一样。
>
>**那么下一个问题，调用next方法会返回什么呢？**
>
>会返回一个对象，里面是本次指向的数据信息。
>
>看下面的例子：
>
>```js
...
>    // console.log(arr);
>    console.log(it)
>    var info1 = it.next(); //{'value':2,'done':false}
>    it.next(); //改变指针指向
>    var info2 = it.next(); //{'value':22,'done':false}
>    var info3 = it.next();
>    var info4 = it.next(); //{'value':'undefinded','done':true}  遍历已结束
>    console.log(info1)
>    console.log(info2)
>    console.log(info3)
>    console.log(info4)
...
>输出：
>
>![mark](http://qiniu.wind-zhou.com/blog/210401/GBCb7gHG0l.png?imageslim)
>
>
...
**对数组和 Set 结构**进行解构赋值时，会默认调用`Symbol.iterator`方法。
...
for (let a of arr) { //其实是相当于使用for...of遍历了arr.values()
...
- 数组的键名是数字，但是`for...in`循环 是以字符串作为键名“0”、“1”、“2”等等。
- `for...in`循环不仅遍历数字键名，还会遍历手动添加的其他键，甚至包括原型链上的键。
- 某些情况下，`for...in`循环会以**任意顺序遍历键名。**
