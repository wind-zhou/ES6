...
上面这种写法跟传统的面向对象语言（比如 C++ 和 Java）差异很大，很容易让新学习这门语言的程序员感到困惑。ES6 提供了更接近传统语言的写法，引入了 Class（类）这个概念，作为对象的模板。通过`class`关键字]\
...
>**这里的this指向和es5一致：即构造函数的里的this指向实例化的对象。**
...
> **注释**：因此，在类的实例上面调用方法，其实就是调用原型上的方法。
...
`constructor()`方法是类的默认**方法**，通过`new`命令生成对象实例时，自动调用该方法。**一个类必须有`constructor()`方法，如果没有显式定义，一个空的`constructor()`方法会被默认添加。**
...
> **`__proto__` 并不是语言本身的特性**，这是各大厂商具体实现时添加的私有属性，虽然目前很多现代浏览器的 JS 引擎中都提供了这个私有属性，但依旧不建议在生产中使用该属性，避免对环境产生依赖。生产环境中，我们可以使用 **`Object.getPrototypeOf`** 方法**来获取实例对象的原型**，然后再来为原型添加方法/属性。

>注释：这里我们得到一个获取对象原型的方法：**`Object.getPrototypeOf`**
...
- **构造函数里边的this指向实例化的对象**
- **方法里的this还是谁调用就指向谁**
- **class里的公共属性和方法，都要加this使用。**



类的方法内部如果含有**`this`，它默认指向类的实例**。但是，必须非常小心，一旦单独使用该方法，很可能报错。

>**注释**：类的方法需要用实例化的对象来调用，单独使用可能会出错（因为this的指向问题）
>
>不过我想，应该没人在编程中这么搞。

![mark](http://qiniu.wind-zhou.com/blog/210406/cAd9kKlIKJ.png?imageslim)



> **注：类里面的公共的属性和方法一定要加this使用。**
