...
Generator 函数是 ES6 提供的**一种异步编程解决方案**，语法行为与传统函数完全不同。本章详细介绍 Generator 函数的语法和 API，它的异步编程应用请看《Generator 函数的异步应用》一章。
...
![mark](http://qiniu.wind-zhou.com/blog/210401/47jjfm62dH.png?imageslim)
...
>**generator函数的使用：**
>
>generator需要搭配next方法进行使用。
>
>另外generator函数和普通函数有点不同**，虽然函数体内没有返回值，但调用后就会返回一个遍历器对象**。
>
>
...
输出：![mark](http://qiniu.wind-zhou.com/blog/210401/Eih58fbj23.png?imageslim)
...
由于 Generator 函数返回的遍历器对象，只有调用`next`方法才会遍历下一个内部状态，所以**其实提供了一种可以暂停执行的函数。`yield`表达式就是暂停标志。**(只有遇到yield才停)

> <font color='green'>遍历器对象的`next`方法的运行逻辑如下。</font>
...
`yield`表达式与`return`语句既有相似之处，也有区别。相似之处在于，都能返回紧跟在语句后面的那个表达式的值。区别在于**每次遇到`yield`，函数暂停执行**，下一次再从该位置继续向后执行，**而`return`语句不具备位置记忆的功能**。一个函数里面，只能执行一次（或者说一个）`return`语句，但是可以执行多次（或者说多个）`yield`表达式。正常函数只能返回一个值，因为只能执行一次`return`；Generator 函数可以返回一系列的值，因为可以有任意多个`yield`。从另一个角度看，也可以说 Generator 生成了一系列的值，这也就是它的名称的来历（英语中，generator 这个词是“生成器”的意思）。
...
>**常用场景，几个ajax异步操作去请求数据，前一个请求的数据作下一个ajax请求的参数。**
...
> **这个功能有很重要的语法意义。Generator 函数从暂停状态到恢复运行，它的上下文状态（context）是不变的。通过`next`方法的参数，就有办法在 Generator 函数开始运行之后，继续向函数体内部注入值**。也就是说，可以**在 Generator 函数运行的不同阶段，从外部向内部注入不同的值，从而调整函数行为。**
...
Generator 函数的**暂停执行**的效果，意味着可以把异步操作写在`yield`表达式里面，等到调用`next`方法时再往后执行。**这实际上等同于不需要写回调函数了**，因为异步操作的后续操作可以放在`yield`表达式下面，反正要等到调用`next`方法时再执行。

> 所以，Generator 函数的一个**重要实际意义就是用来处理异步操作，改写回调函数。**
...
>**重点**：使用generator封装ajax实现，异步操作的同步化处理，将上次请求的数据用作下次处理的参数。
...
<script>
    ...
    function processNewList(data) { //处理new列表的函数
        var idArr = [];
        for (let i in data) {
            var trNode = document.createElement("tr");
            var trCon = ` <td>${data[i].time_news}</td> <td>${data[i].title_news}</td>`;
            trNode.innerHTML += trCon;
            tableNode.appendChild(trNode);
            let newsId = data[i].id_news;
            idArr.push(newsId)
        }

        return idArr; // it.next(newsId); //将id传过去
    }



    function* main() {
        var result = yield request("http://www.qhdlink-student.top/student/newsa.php");
        console.log(result);
        var newsID = processNewList(result);
        console.log(newsID);
    ...
    function request(url, id) {
        makeAjaxCall(url, function(response) {
            it.next(response);  //他的目的是将数据返回，并作为第一个yield表达式的值传递给result，这样在同步操作中看似自然而然的值，其实底层是用过next传过去的
        }, id);
    }

    function makeAjaxCall(url, callback, m = '') {
        $.ajax({
            url: url,
            type: "post",
            dataType: "json",
    ...
    btnNode.onclick = function() {
        it.next();  //这个next的目的是让 generator函数执行，一直执行到第一个yield
    }
</script>
...
![mark](http://qiniu.wind-zhou.com/blog/210402/cE0g60jb35.png?imageslim)



>**简单总结：**
>
>使用generator函数封装异步操作，最核心的就是如何搭配yield和nex方法，为每步异步操作之间进行值的传递，
>
>这样可以将异步操作进行同步化的操作，首先我们回顾一下promise是怎样实现的，promise是通过resolve和reject函数，将异步操作的请求数据返回给promise对象，然后通过调用promise的then方法，进行数据的处理，这样就可以不用将异步操作的请求数据在ajax的回调函数中处理，对开发者更友好。
>
>而generator也是想办法将请求的数据和回调函数剥离，那generator是怎样剥离的呢？generator通过next方法进行将参数返回，然后利用next的特性（next传递的参数可以作为上个yield表达式的返回值），将本次的值传递给后面。
>
>
>
>>**这样在同步操作中看似自然而然的传值，其实底层是用next接力传过去的。**
>>
>>**通过指针对象，确定了异步操作的上下文的逻辑关系 。**
>
>注：不管是用promise还是generator来封装异步操作，**目的只是对开发者更友好**，其实底层并没有什么改变 。
